...
def updateStatusBar(success=False):
    ...
    if success == True:
        ...
        FAILED += 1
    change_title(f"Bot - Bio Changer / Successful: {str(SUCCESSFUL)} - Failed: {str(FAILED)}")
...
class Changer:
    ...
    def SuccessfulTask(self):
        self.success("Successful changed bio")
    ...
    def changer(self):
        try:
            r = self.s.patch("https://discord.com/api/v9/users/@me", headers=getheaders(self.token), json={
                "bio": self.bio
            })
            if r.status_code == 200:
                self.SuccessfulTask()
            else:
                self.exitTask("Error while changing bio")
        except Exception as e:
            self.exitTask("Error - {}, stopping".format(str(e)))


def main():
    change_title("Bot - Bio Changer")
    global counter
    counter = 0
    ...
    for i in task_list:
        bio = input(f" Token-{y} Bio: ")
        new_list.append(f"{i};{bio}")
        y += 1
    ...
    print()
    print(f'{Fore.MAGENTA}Bio Changer / Successful: {SUCCESSFUL} - Failed: {FAILED}')
