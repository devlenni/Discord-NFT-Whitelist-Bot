...
def updateStatusBar(success=False):
    ...
    if success == True:
        ...
        FAILED += 1
    change_title(f"Bot - Hypesquad Changer / Successful: {str(SUCCESSFUL)} - Failed: {str(FAILED)}")
...
class Changer:
    ...
    def SuccessfulTask(self):
        self.success("Successful changed house")
    ...
    def changer(self):
        r = self.s.post("https://discord.com/api/v9/hypesquad/online", headers=self.headers, json={"house_id": random.randint(1, 3)})
        if r.status_code == 200:
            ...
            self.exitTask("Error while changing house")


def main():
    change_title("Bot - Hypesquad Changer")
    global counter
    counter = 0
    ...
    print()
    print(f'{Fore.MAGENTA}Hypesquad Changer / Successful: {SUCCESSFUL} - Failed: {FAILED}')
