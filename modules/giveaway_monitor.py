...
def updateStatusBar(success=False):
    ...
    if success == True:
        ...
        FAILED += 1
    change_title(f"Bot - Giveaway Monitor / Successful: {str(SUCCESSFUL)} - Failed: {str(FAILED)}")
                        ...
                        if len(self.last_message) == 0:
                            ...
                            if data["embeds"] and "react with" and "to enter!" in str(data["embeds"][0]["description"]).lower():
                                ...
                                if "congratulations" or "you won" in str(data).lower():
                                    ...
                                    pass
                        elif m_id != self.last_message[-1]:
                            self.last_message.append(m_id)
                            if data["embeds"] and "react with" and "to enter!" in str(
                                    data["embeds"][0]["description"]).lower():
                                self.status("Found a giveaway")
                                self.join(data)
                            elif data["embeds"] and data["components"]:
                                if "entries" and "winners" in str(data["embeds"][0]["description"]).lower():
                ...
                try:
                    if len(self.last_message) == 0:
                        for i in data:
                            ...
                            self.last_message.append(m_id)
                            if i["embeds"]:
                                if "react with" and "to enter!" in str(i["embeds"][0]["description"]).lower():
                                    self.status("Found a giveaway")
                                    self.join(i)

                            elif i["embeds"] and i["components"]:
                                if "entries" and "winners" in str(i["embeds"][0]["description"]).lower():
                                    self.status("Found a giveaway")
                                    self.join_button_based(i)

                            else:
                                if "congratulations" or "you won" in str(i).lower():
                                    profile_url = "https://discord.com/api/v9/users/@me"
                                    try:
                                        r = self.s.get(profile_url, headers=self.headers)
                                        profile_id = r.json()["id"]
                        ...
                        for i in data:
                            m_id = i["id"]
                            if m_id not in self.last_message:
                                self.last_message.append(m_id)

                                if i["embeds"] and "react with" and "to enter!" in str(i["embeds"][0]["description"]).lower():
                                    self.status("Found a giveaway")
                                    self.join(i)
...
def main():
    change_title("Bot - Giveaway Monitor")
    global counter
    counter = 0
    config = Config()
    ...
    print()
    print(f'{Fore.MAGENTA}Giveaway Monitor / Successful: {SUCCESSFUL} - Failed: {FAILED}')
